        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-0 border border-border/50 rounded-2xl overflow-hidden bg-card/30 backdrop-blur-xl shadow-lg">
          {[
            ...
            <div key={label} className={`p-4 md:p-6 text-center ${idx !== arr.length - 1 ? 'border-r border-border/50' : ''}`}>
              <span className="text-2xl md:text-3xl font-black text-primary drop-shadow-sm">{value}</span><span className="text-md md:text-xl font-black text-foreground drop-shadow-sm">{unit}</span>
          ...
          <div className="flex gap-2 p-1 bg-muted rounded-lg w-fit mx-auto">
            {[
              ...
              <button
                key={id}
                onClick={() => setTab(id)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold transition-all ${tab === id ? 'bg-background text-foreground shadow-lg shadow-background/20 backdrop-blur-md' : 'text-muted-foreground hover:text-foreground'
                  }`}
              >
                          ...
                          <CardContent className="p-4 flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-4 md:gap-8 text-right">
                              ...
                              <div className="hidden sm:block">
                                <p className="text-sm font-black text-green-500 line-clamp-1">{act.co2SavedKg?.toFixed(2)} kg <span className="text-sm text-muted-foreground font-normal">CO<sub>2</sub></span></p>
                              </div>
                            </div>
                          </CardContent>
                        </Card>
            ...
            <Card className="border-border/50 bg-card/40 backdrop-blur-xl shadow-lg">
              <CardHeader className="border-b border-border/30 bg-background/20">
                <CardTitle className="text-base flex items-center gap-2">
                  <BarChart2 className="w-4 h-4 text-primary" /> Last 7 Days — CO₂ Saved (kg)
                </CardTitle>
              </CardHeader>
              <CardContent>
              ...
              <Card className="border-border/50 bg-card/40 backdrop-blur-xl shadow-lg">
                ...
                <CardContent className="flex flex-wrap gap-3">
                  {breakdownData.map(({ mode, count }) => {
                    ...
                    const Icon = cfg.icon;
                    return (
                      <div key={mode} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/5 backdrop-blur shadow-sm my-4 ${cfg.bg}`}>
                        <Icon className={`w-5 h-5 ${cfg.color} drop-shadow-sm`} />
                        <span className="text-sm font-bold">{cfg.label}</span>
                        <span className="text-xs font-black  px-2 py-0.5 rounded-md text-foreground">{count}x</span>
                      </div>
                    );
                  })}
                </CardContent>
