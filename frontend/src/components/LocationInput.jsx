...
// Global cache to persist results across component re-renders
  ...
  useEffect(() => {
    const fetchPlaces = async () => {
      ...
      if (locationCache.has(query.toLowerCase())) {
        setSuggestions(locationCache.get(query.toLowerCase()));
        setIsOpen(true);
        return;
      }
      ...
      try {
        ...
        // 2. Store in Cache (Limit size to 100 to prevent memory leaks)
        if (locationCache.size > 100) {
          const firstKey = locationCache.keys().next().value;
          locationCache.delete(firstKey);
        }
        locationCache.set(query.toLowerCase(), parsed);
    ...
    <div className="w-full space-y-3 relative" ref={dropdownRef}>
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="px-10 text-base bg-background/50 border-border shadow-inner"
          value={query}
